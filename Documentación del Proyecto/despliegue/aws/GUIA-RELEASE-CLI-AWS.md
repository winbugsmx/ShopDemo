...
docker build -t shopdemo-catalog:$IMAGE_TAG -f Source/Catalog/ShopDemo.Catalog.Api/Dockerfile Source
docker tag shopdemo-catalog:$IMAGE_TAG "$ECR/shopdemo-catalog:$IMAGE_TAG"
docker push "$ECR/shopdemo-catalog:$IMAGE_TAG"

docker build -t shopdemo-orders:$IMAGE_TAG -f Source/Orders/ShopDemo.Orders.Api/Dockerfile Source
docker tag shopdemo-orders:$IMAGE_TAG "$ECR/shopdemo-orders:$IMAGE_TAG"
docker push "$ECR/shopdemo-orders:$IMAGE_TAG"

docker build -t shopdemo-inventory:$IMAGE_TAG -f Source/Inventory/ShopDemo.Inventory.Api/Dockerfile Source
...
Replica el release validado: **4 nodos `t3.micro`**, Source/Catalog, Source/Orders, Source/Inventory con LoadBalancer y Swagger en **:8080**, sin MCP/Ingress/Analytics.
