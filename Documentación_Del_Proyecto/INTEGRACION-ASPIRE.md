...
**Estado del código:** Aspire + Analytics.Api en `Source/Aspire/` — ver [Documentación_Del_Proyecto/analytics/](./analytics/)
...
- **No requiere HTTP** hacia Source/Catalog, Source/Orders, Source/Inventory — cero acoplamiento nuevo.
...
1. Alumno ejecuta **un solo comando**: `dotnet run --project Source/Aspire/ShopDemo.AppHost`
...
**Referencia en solución:** carpeta `/Aspire/` en `Source/ShopDemo.slnx` — los 13 proyectos actuales intactos.
...
```bash
cd I:\Curso\ShopDemo
dotnet new aspire-servicedefaults -n ShopDemo.ServiceDefaults -o Source/Aspire/ShopDemo.ServiceDefaults
```
...
```bash
dotnet new aspire-apphost -n ShopDemo.AppHost -o Source/Aspire/ShopDemo.AppHost
```
...
```bash
dotnet add Source/Aspire/ShopDemo.AppHost/ShopDemo.AppHost.csproj reference \
  Source/Catalog/ShopDemo.Catalog.Api/ShopDemo.Catalog.Api.csproj

dotnet add Source/Aspire/ShopDemo.AppHost/ShopDemo.AppHost.csproj reference \
  Source/Orders/ShopDemo.Orders.Api/ShopDemo.Orders.Api.csproj

dotnet add Source/Aspire/ShopDemo.AppHost/ShopDemo.AppHost.csproj reference \
  Source/Inventory/ShopDemo.Inventory.Api/ShopDemo.Inventory.Api.csproj

dotnet add Source/Aspire/ShopDemo.AppHost/ShopDemo.AppHost.csproj reference \
  Source/Aspire/ShopDemo.Analytics.Api/ShopDemo.Analytics.Api.csproj
```
...
Reutiliza `IntegrationEventEnvelope` de `ShopDemo.Shared` (mismo contrato que Source/Catalog, Source/Orders, Source/Inventory).
...
### Paso 8 — Agregar a Source/ShopDemo.slnx
...
```bash
cd Source/Aspire/ShopDemo.AppHost
dotnet run
```
...
```bash
dotnet publish Source/Aspire/ShopDemo.AppHost -p:PublishProfile=DefaultContainer
```
...
| ¿Implementación sencilla? | **Sí** — 3 proyectos nuevos en `Source/Aspire/`, AppHost referencia APIs existentes |
| ¿Estado actual? | ✅ **Implementado** — ver `Source/Aspire/` y [Documentación_Del_Proyecto/analytics/](./analytics/) |
