"""Actualiza rutas Documentación_De_Estudio_Del_Curso -> Documentación_De_Estudio_Del_Curso."""
from __future__ import annotations

from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parents[2]
OLD = "Documentación_De_Estudio_Del_Curso"
NEW = "Documentación_De_Estudio_Del_Curso"
...
def fix_text(text: str) -> str:
    pairs = [
        ...
        ("Join-Path $Root '" + OLD + "'", "Join-Path $Root '" + NEW + "'"),
        ("(Join-Path $RepoRoot '" + OLD + "')", "(Join-Path $RepoRoot '" + NEW + "')"),
        (f"globs: {OLD}/**", f"globs: {NEW}/**"),
        ("Documentación_De_Estudio_Del_Curso/", f"{NEW}/"),
        ("Documentación_De_Estudio_Del_Curso", NEW),
        (OLD, NEW),
    ]
