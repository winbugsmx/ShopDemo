...
- `dotnet build ShopDemo.slnx` compila sin errores.
...
- Postman importado desde `docs/ShopDemo.postman_collection.json`.
...
```
ShopDemo/
├── Catalog/          # Catálogo (Clean + CQRS)
├── Orders/           # Pedidos (Clean + CQRS)
├── Inventory/        # Inventario (hexagonal)
├── AI/               # MCP Gateway
├── Aspire/           # AppHost, ServiceDefaults, Analytics
├── ShopDemo.Shared/  # Kernel compartido
