...
| Comando | Descripción | Ejemplo |
|---|---|---|
| `docker build` | Construye una imagen desde Dockerfile | `docker build -t shopdemo-catalog:1.0 -f ShopDemo.Catalog.Api/Dockerfile .` |
...
```bash
cd I:\Curso\ShopDemo\ShopDemo.Catalog.Api
docker compose up -d
```
...
```bash
docker build -t myregistry.azurecr.io/shopdemo-catalog:1.0 -f ShopDemo.Catalog.Api/Dockerfile .
