...
| `dotnet sln add` | Agrega un proyecto a la solución | `dotnet sln ShopDemo.slnx add ShopDemo.Catalog.Api/ShopDemo.Catalog.Api.csproj` |
...
| Comando | Descripción | Ejemplo |
|---|---|---|
| `dotnet restore` | Descarga paquetes NuGet | `dotnet restore ShopDemo.slnx` |
| `dotnet build` | Compila la solución o proyecto | `dotnet build ShopDemo.slnx -c Release` |
...
| `dotnet clean` | Elimina artefactos de compilación | `dotnet clean ShopDemo.slnx` |

**Ejemplo completo — levantar Catalog.Api:**

```bash
cd I:\Curso\ShopDemo
dotnet restore ShopDemo.slnx
...
| `dotnet test` | Ejecuta pruebas | `dotnet test ShopDemo.slnx` |
